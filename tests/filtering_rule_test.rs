use pg_filters::filtering::{ColumnName, ConditionalOperator, FilterOperator, FilteringRule, FilterColumn};

#[test]
fn test_filtering_rule() {
    let filtering_rule = FilteringRule::new("and".into(), ColumnName::String("name"), "=".into(), "John".into()).unwrap();

    assert_eq!(filtering_rule.filter_column, FilterColumn::String("name", "'John'".to_string()));
...
fn test_filtering_rule_with_int_value() {
    let filtering_rule = FilteringRule::new("or".into(), ColumnName::Int("age"), ">".into(), "18".into()).unwrap();
...
fn test_filtering_rule_with_float_value() {
    let filtering_rule =
        FilteringRule::new("and".into(), ColumnName::Float("height"), ">=".into(), "5.5".into()).unwrap();

    assert_eq!(filtering_rule.filter_column, FilterColumn::Float("height", 5.5));
    assert_eq!(
...
fn test_filtering_rule_with_bool_value() {
    let filtering_rule =
        FilteringRule::new("or".into(), ColumnName::Bool("is_active"), "=".into(), "true".into()).unwrap();

    assert_eq!(filtering_rule.filter_column, FilterColumn::Bool("is_active", true));
    assert_eq!(filtering_rule.filter_operator, FilterOperator::Equal);
    assert_eq!(filtering_rule.conditional_operator, ConditionalOperator::Or);
}
...
fn test_filtering_rule_with_null_value() {
    let filtering_rule =
        FilteringRule::new("and".into(), ColumnName::String("email"), "is null".into(), "".into()).unwrap();

    assert_eq!(filtering_rule.filter_column, FilterColumn::String("email", "".into()));
...
fn test_filtering_rule_with_not_null_value() {
    let filtering_rule =
        FilteringRule::new("or".into(), ColumnName::String("email"), "is not null".into(), "".into()).unwrap();
...
fn test_filtering_rule_with_like_value() {
    let filtering_rule =
        FilteringRule::new("and".into(), ColumnName::String("name"), "like".into(), "John".into()).unwrap();

    assert_eq!(filtering_rule.filter_column, FilterColumn::String("name", "'%John%'".into()));
...
fn test_filtering_rule_with_not_like_value() {
    let filtering_rule =
        FilteringRule::new("or".into(), ColumnName::String("name"), "not like".into(), "John".into()).unwrap();
...
fn test_filtering_rule_with_string_in_value() {
    ...
    assert_eq!(filtering_rule.filter_column, FilterColumn::StringList("name", vec!["'John'".into(), "'Jane'".into()]));
...
fn test_filtering_rule_with_int_in_value() {
    ...
    assert_eq!(filtering_rule.filter_column, FilterColumn::IntList("name", vec![1,2,3]));
    assert_eq!(filtering_rule.filter_operator, FilterOperator::In);
    assert_eq!(
        filtering_rule.conditional_operator,
        ConditionalOperator::And
...
fn test_filtering_rule_with_float_in_value() {
    ...
    assert_eq!(filtering_rule.filter_column, FilterColumn::FloatList("name", vec![1.1,2.2,3.0]));
    assert_eq!(filtering_rule.filter_operator, FilterOperator::In);
    assert_eq!(
        filtering_rule.conditional_operator,
        ConditionalOperator::And
...
fn test_filtering_rule_with_bool_in_value() {
    ...
    assert_eq!(filtering_rule.filter_column, FilterColumn::BoolList("name", vec![true, false]));
    assert_eq!(filtering_rule.filter_operator, FilterOperator::In);
    assert_eq!(
        filtering_rule.conditional_operator,
        ConditionalOperator::And
...
fn test_filtering_rule_with_invalid_conditional_operator() {
    let filtering_rule = FilteringRule::new("}".into(), ColumnName::String("name"), "=".into(), "John".into()).unwrap();
